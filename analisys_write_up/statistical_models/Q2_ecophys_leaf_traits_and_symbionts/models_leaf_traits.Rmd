...
vars <- names(data_rate_change)[18:(ncol(data_rate_change)-1)]
```
...
```{r}
data_d13c_plot <- emmip(mixed_models$d13c, treatment ~ nfixer, 
						CIs = TRUE)$data
...
```{r}
data_sla_plot <- emmip(mixed_models$sla_cm2_g, treatment ~ nfixer, 
					   CIs = TRUE)$data
...
```{r}
Anova(mixed_models$narea_g_m2, type = "III",test="F")
```
...
```{r}
Anova(mixed_models$nmass_mg_g, type = "III",test="F")
```
...
## Canopy Nitrogen Content (this model was droped) 
```{r}
#Anova(mixed_models$totalleafmass_nmass, type = "III")
```

```{r}
#data_leafmass_nmass_plot <- emmip(mixed_models$totalleafmass_nmass, treatment ~ nfixer,
#CIs = TRUE)$data
...
## sqrt Canopy Nitrogen Content  (THis model was selected)
```{r}
#totalleafmass_nmass_sqrt <- 
#	lmer(sqrt(totalleafmass_nmass) ~  treatment * nfixer +
#								 	init_height + 
#								 	(1|spcode), 
...
```{r}
#Anova(totalleafmass_nmass_sqrt, type = "III",test="F")
```

```{r}
#data_leafmass_nmass_plot_sqrt <- emmip(totalleafmass_nmass_sqrt, 
#									   treatment ~ nfixer, CIs = TRUE)$data

# Get by group comparitions
#    as.data.frame(emmeans(totalleafmass_nmass_sqrt,
#                          pairwise ~ treatment | nfixer,
#                          type = "response", 
#                          adjust ="tukey")$contrast) %>% 
...
```{r}
#(plot_totalleafmass_nmass_sqrt <- 
#  masomenos_plot(data = data_leafmass_nmass_plot_sqrt,
#                 xvar = xvar, yvar = yvar, 
#                 tvar = tvar, color = treatment,
#                 ucl = UCL, lcl = LCL) +
  
#   theme(legend.position = "right" ) + 
#  ylab("sqrt(Canopy Nitrogen Content (Total leaf mass * nmass))"))
