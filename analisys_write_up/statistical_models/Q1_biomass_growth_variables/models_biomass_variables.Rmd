...
as.data.frame(emmeans(varident_belowbiom,
    ...
    mutate(group1 = factor(group1)) %>%
    mutate(group2 = factor(group2)) %>% 
	dplyr::select(group1, group2,nfixer,estimate,df,se,t_ratio,p_value) %>%
    mutate(p_value = round(p_value, 20)) %>% 
    kable()
...
```{r}
(emm_total_biomass_fixers_nonfixers <- emmeans(mixed_models$totalbiom, 
											   specs = "nfixer", by="treatment"))
```
