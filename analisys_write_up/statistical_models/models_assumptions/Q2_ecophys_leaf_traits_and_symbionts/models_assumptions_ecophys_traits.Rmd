...
mixed_models_formula = function(response) {
  ...
  glmer(as.formula(formula), family = gaussian(link = "identity"), 
...
```{r}
testDispersion(simulation_output_amax)
```
...
```{r}
testDispersion(simulation_output_gs)
```
...
```{r}
testDispersion(simulation_output_wue)
```
