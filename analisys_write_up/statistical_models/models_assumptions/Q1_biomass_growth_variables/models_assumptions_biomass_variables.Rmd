...
knitr::opts_chunk$set(echo = TRUE, comment = "", fig.align='center',fig.width=16, 
					  fig.height=8)
```
...
```
```{r}
plot(data_rate_change$treatment, resid(mixed_models$below_biom), xlab = "Treatment",
ylab = "Residuals")
```

### First model Belowground biomass model
...
```{r}
E <- resid(varident_belowbiom)
coplot(E ~ treatment | nfixer, data = data_rate_change )
```
...
```{r}
E <- resid(varident_rmf,type = "normalized")
coplot(E ~ treatment | nfixer, data = data_rate_change, 
...
plot(smf_model,which = c(1), col = data_rate_change$treatment, 
	 add.smooth = T, caption = "Treatment")
```

```{r}
plot(smf_model,which = c(1), col = data_rate_change$nfixer, 
	 add.smooth = T, caption = "nfixer")
```
...
```{r}
E <- resid(smf_model,type = "normalized")
coplot(E ~ treatment | nfixer, data = data_rate_change, 
	   ylab = "Normalised residuals" )
```
