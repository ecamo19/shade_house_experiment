...
fviz_pca_biplot(pca_final_traits,axes = c(1, 3),
                ...
                # Add group 
                habillage=all_trait_scaled$treatment, 
...
compare_performance(varident_pca_mod_agr_two_way, varident_pca_mod_agr_three_way,metrics = "all")
```

```{r}
anova(varident_pca_mod_agr_two_way, varident_pca_mod_agr_three_way)
```
...
Anova(varident_pca_mod_agr_two_way, type = "III",test = "F") 
```
...
# check two-way vs. three-way interaction models 
# (shows that two-way is way better, more negative AICc -- include in methods)
compare_performance(varident_pca_mod_totalbiom_two_way, varident_pca_mod_totalbiom_three_way,metrics = "all")
```

```{r}
anova(varident_pca_mod_totalbiom_two_way, varident_pca_mod_totalbiom_three_way)
```
...
```{r message=FALSE, warning=FALSE}
# ANOVA -- PC1 X Treatment effect significant (chi-sq = 29.25, P <0.001) 
Anova(varident_pca_mod_totalbiom_three_way, type = "III",test="F") %>% 
  kable()
```
